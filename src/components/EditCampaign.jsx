...
const EditCampaign = ({ isOpen, onClose, campaign, onSubmit }) => {
  ...
  useEffect(() => {
    if (campaign) {
      setForm({
        ...
        kpi: campaign.kpi || "",
        kpiAchieved: campaign.kpiAchieved || "no",
        btag: campaign.btag || "",
        btagLogin: campaign.btagLogin || "",
        btagPassword: campaign.btagPassword || "",
        ...
        payments: campaign.payments?.length > 0 ? campaign.payments : [{ date: "", amount: "" }],
        campaignLinks: campaign.campaignLinks?.length > 0 ? campaign.campaignLinks : [{ heading: "", url: "" }]
      });
    }
  }, [campaign]);
  ...
  const removePaymentRow = (index) => {
    ...
    setForm({ ...form, payments: updated.length > 0 ? updated : [{ date: "", amount: "" }] });
  };
  ...
  const removeLinkRow = (index) => {
    ...
    setForm({ ...form, campaignLinks: updated.length > 0 ? updated : [{ heading: "", url: "" }] });
  };
      ...
      <div className="bg-white p-6 rounded-xl w-[700px] overflow-y-scroll max-h-[90vh] shadow-2xl">
        <div className="sticky top-0 bg-white pb-4 border-b mb-4">
          <h2 className="text-2xl font-bold text-gray-800">
            Edit Campaign: {campaign?.campaignName}
          </h2>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="border border-gray-300 p-2.5 w-full rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-gray-100"
        ...
        <div className="flex justify-between mt-6 pt-4 border-t sticky bottom-0 bg-white">
          ...
          <button
            className="bg-purple-600 text-white px-6 py-2.5 rounded-lg hover:bg-purple-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center gap-2"
